...
def show_contest(user_var, pd_var):
    ...
    for name, con_id in cur_list:
        ...
        tk.Button(window,text=name, font=3, height=1,command=partial(enter_contest, con_id, user, pd)).pack()
    window.mainloop()
    return
...
def enter_contest(con_id, user, pd):
    ...
    for each in pro_name_list:
        ...
        pro_list.append([name, lim, problem])
...
def show_problems(s:requests.Session, CID, pro_name_list, pro_list, user, pd):
    ...
    for name, lim, pro_info in pro_list:
        tk.Label(frame1, text="").pack()
        tk.Button(frame1, text=name, command=partial(
            pro_window, name, lim, pro_info), font=3, height=1).pack()
    ...
    lan_b.pack()
    b = tk.Button(frame3_r, text="Submit", command=lambda:submit(s, CID, pro_name_list,user, pd, e_pro, e_file))
    b.pack()
    window.mainloop()
    return
